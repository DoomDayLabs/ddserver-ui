...
class Пахом extends React.Component{
    ...
    render(){        
        return  <Snackbar 
                    ...
                    timeout={3000}
                    onTimeout={()=>this.setState({action:false})}                         
                > 
                    <div className={style.snackbar}>
                        <span>{this.state.title}</span>                        
                        <div>
                            <Button icon={this.state.actionIcon} label={this.state.action} inverse onClick={()=>this.handleClick()}/>
                        </div>
                    </div>
...
class App extends React.Component{
    ...
    handleSendWidgetTest(){
        ...
        post({
            url:'http://localhost:8080/dds/admin/widgets/add',
            body:formData
        }).then(console.log);
    }
    render(){
        ...
        return (
            <Layout>
                ...
                <Panel>     
                    <Button label='SEND!' onClick={()=>this.handleSendWidgetTest()} />               
